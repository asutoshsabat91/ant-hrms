...
export async function GET() {
  try {
    ...
    console.error("[POSH GET]", e);
    return NextResponse.json({ error: "Failed to fetch POSH reports" }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    console.error("[POSH POST]", e);
    return NextResponse.json({ error: "Failed to submit POSH report" }, { status: 500 });
  }
}

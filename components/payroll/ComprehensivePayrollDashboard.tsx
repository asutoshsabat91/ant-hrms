...
export function ComprehensivePayrollDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "payslips" | "breakdown">("overview");
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex gap-1 rounded-xl border border-zinc-100 bg-zinc-50 p-1 w-fit">
            {(["overview", "payslips", "breakdown"] as const).map((tab) => (
              <button
                key={tab}
              ...
              >
                {tab === "overview" ? "Overview" : tab === "payslips" ? "Employee Payslips" : "Salary Breakdown"}
              </button>
            ))}
          </div>

...
export function SettingsPageClient({
  ...
  const [activeTab, setActiveTab] = useState<"general" | "leaves">("general");
  ...
  const handleToggleApplicable = (val: string) => {
    ...
    const current = editingType.applicableTo || [];
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!editingType.applicableTo || editingType.applicableTo.length === 0) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex border-b border-zinc-200">
        <button
          ...
          className={`pb-3 px-4 text-sm font-semibold transition-all duration-200 border-b-2 ${
            activeTab === "general"
      ...
      {activeTab === "leaves" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-bold text-zinc-900">Leave Policies</h3>
            ...
            {isSuperAdmin && (
              <button
                ...
                className="flex items-center gap-1.5 px-3.5 py-2 bg-zinc-900 hover:bg-zinc-800 text-white rounded-lg text-xs font-bold transition shadow-sm"
              >
                        ...
                        <td className="p-3 text-right">
                          <div className="inline-flex items-center gap-1.5">
                            <button
                              ...
                              className="p-1 hover:bg-zinc-100 text-zinc-600 hover:text-zinc-950 rounded transition"
                              title="Edit policy"
                            >
                              <Edit size={14} />
                            </button>
                            {deleteConfirmId === lt.id ? (
                              <div className="inline-flex items-center gap-1 bg-red-50 border border-red-200 rounded px-1.5 py-0.5 text-[10px] font-bold text-red-700">
                                ...
                                <button
                                  ...
                                  className="hover:underline text-red-800 ml-1"
                                >
                                  Yes
                                </button>
                                <button
                                  onClick={() => setDeleteConfirmId(null)}
                                  className="hover:underline text-zinc-500 ml-1"
                                >
                                  No
                                </button>
                              </div>
                              ...
                              <button
                                ...
                                className="p-1 hover:bg-zinc-100 text-zinc-500 hover:text-red-600 rounded transition"
                                title="Delete policy"
                              >
                                <Trash2 size={14} />
                              </button>
          ...
          >
            <div className="border-b border-zinc-100 pb-3 mb-4">
              <h3 className="text-base font-bold text-zinc-950">
                {editingType.id ? "Edit Leave Policy" : "Create Leave Policy"}
              </h3>
              <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-wide mt-0.5">
                {editingType.id ? "Modify configuration details" : "Add a new dynamic leave type"}
              </p>
            </div>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border border-zinc-200 rounded-lg p-2 text-sm font-medium focus:outline-none"
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  disabled={!!editingType.id} // Disable changing code on edit to protect reference integrity
                  placeholder="e.g. EARNED"
                  value={editingType.code || ""}
                  onChange={(e) => setEditingType({ ...editingType, code: e.target.value.toUpperCase().replace(/\s+/g, "_") })}
                  className="w-full border border-zinc-200 rounded-lg p-2 text-sm font-mono font-semibold focus:outline-none disabled:bg-zinc-50 disabled:text-zinc-400"
                />
                {!editingType.id && (
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    min="0"
                    required
                    value={editingType.daysPerYear ?? 0}
                    onChange={(e) => setEditingType({ ...editingType, daysPerYear: parseInt(e.target.value) || 0 })}
                    className="w-full border border-zinc-200 rounded-lg p-2 text-sm font-semibold focus:outline-none"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-zinc-500 mb-1">Accrual Frequency</label>
                  <select
                    value={editingType.accrual || "ANNUAL"}
                    onChange={(e) => setEditingType({ ...editingType, accrual: e.target.value })}
                    className="w-full border border-zinc-200 rounded-lg p-2 text-sm font-medium focus:outline-none"
                  >
              ...
              <div>
                ...
                <input
                  ...
                  value={editingType.priorNoticeHours ?? 0}
                  onChange={(e) => setEditingType({ ...editingType, priorNoticeHours: parseInt(e.target.value) || 0 })}
                  className="w-full border border-zinc-200 rounded-lg p-2 text-sm font-semibold focus:outline-none"
                />
                <p className="text-[9px] text-zinc-400 mt-1 font-medium">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {EMPLOYMENT_TYPES.map((type) => {
                    const isChecked = editingType.applicableTo?.includes(type.value) || false;
                    return (
                      <label
                        key={type.value}
              ...
              <div>
                <label className="flex items-center gap-2 border border-zinc-200 rounded-lg p-2.5 cursor-pointer hover:bg-zinc-50 text-xs font-semibold text-zinc-800">
                  <input
                    type="checkbox"
                    checked={editingType.isPaid ?? true}
              ...
              <div className="flex justify-end gap-2.5 pt-3 border-t border-zinc-100">
                <button
                  type="button"
                  onClick={() => setModalOpen(false)}
                  className="px-4 py-2 border border-zinc-200 text-zinc-700 font-bold text-xs rounded-lg hover:bg-zinc-50 transition"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-4 py-2 bg-zinc-900 text-white font-bold text-xs rounded-lg hover:bg-zinc-800 transition disabled:opacity-50"
                >

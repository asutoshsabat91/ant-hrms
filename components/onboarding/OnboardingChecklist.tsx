...
import { CircleCheck, Clock3, CalendarDays, User } from "lucide-react";
...
const categoryOrder = ["DOCUMENTATION", "IT_SETUP", "ORIENTATION", "COMPLIANCE", "ASSET", "TRAINING", "SOCIAL"];
...
export function OnboardingChecklist({ initialTasks }: OnboardingChecklistProps) {
  const [tasks, setTasks] = useState(initialTasks);
  const [loadingTaskId, setLoadingTaskId] = useState<string | null>(null);

  const groupedTasks = useMemo(() => {
    return tasks.reduce<Record<string, OnboardingTask[]>>((acc, task) => {
        ...
        return (
          <section key={category} className="rounded-3xl border border-[var(--card-border)] bg-[var(--card-bg)] p-5 shadow-[var(--card-shadow)]">
            ...
            <div className="space-y-4">
              {items.map((task) => {
                const due = task.dueDate ? format(parseISO(task.dueDate), "dd MMM yyyy") : "No due date";
                return (
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        ...
                        <div className="mt-2 flex flex-wrap gap-2 text-xs text-[var(--neutral-500)]">
                          ...
                          <span className="inline-flex items-center gap-1"><CalendarDays className="h-3.5 w-3.5" /> {due}</span>
                        </div>
                      </div>
                      <div className="flex flex-col gap-2 sm:items-end">
                        ...
                        {task.status !== "COMPLETED" ? (
                          ...
                          <div className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-3 py-1 text-xs text-emerald-700">
